...
function getIngredients(productsList, productChosen) {
  //code
  const result = productsList.filter((item) => productChosen === item.name);
...
function getIngredientsCaseInsensitive(productsList, productChosen) {
  ...
  const result = productsList.filter(
    (item) => productChosen === item.name.toLowerCase()
  );
  return result[0].ingredients;
}
...
function checkProduct(productsList, productChosen) {
  // code
  const check = productsList.some(
    (item) => productChosen.toLowerCase() === item.name.toLowerCase()
  );
  return check;
}
...
function getProductInStock(productsList, productChosen) {
  ...
  const check = productsList.some(
    (item) =>
      productChosen.toLowerCase() === item.name.toLowerCase() &&
      item.quantity > 0
  );
  return check;
}
...
function getProducts(productsList, productChosen) {
  ...
  const search = productsList.filter((item) =>
    item.name.toLowerCase().includes(productChosen)
  );
  return search;
}
...
function excludeIngredient(productList, ingredient) {
  ...
  const exclu = productList.filter(
    (item) => !item.ingredients.toString().toLowerCase().includes(ingredient.toLowerCase())
  );
  return exclu;
}
